...
// import "../styles.scss";
import chain from "../chainCore/chain";
...
function App() {
  ...
  async function connectOrUpdateWallet() {
    ...
    const address = await chain.getUserAddress();
    setAddress(address);
  }

  async function updateOwnedNfts() {
    const address = await chain.getUserAddress();
    const tokenIds = await chain.getOwnedNfts(address);
    setOwnedNfts(tokenIds.map((tokenId) => tokenId.toNumber()));
  }

  async function updateOrders() {
    const orders = await chain.getAllOrders();
    setOrders(orders);
  }
  ...
  return (
    ...
    >
      <div className="App">
          ...
          <div id="nft-secion">
            ...
            <div className="cards-container">
              ...
              <a
                href="#"
                onClick={mintNft}
                className="button mint-new-nft-link"
              >
                + Mint new NFT
              </a>
            </div>
        ...
        {/* LOGO */}
        <div id="logo">NFT Marketplace</div>

        {/* WALLET SELECTOR */}
        <div id="wallet-connector">
          {address ? (
            chain.shortenAddress(address)
          ) : (
            <a className="button" href="#" onClick={chain.connectWallet}>
              Connect wallet
            </a>
          )}
        </div>
